...
4. **Handoff:** Update `.gemini/state/ticket-<id>.json`. Set `last_agent: "doc-agent"` and store status (e.g., `docs_updated: true`, `pr_created: true`) inside a `"doc-agent"` key. You MUST then hand off to the **Main Agent** for manual review and approval before the task can proceed to the `project-agent`.
