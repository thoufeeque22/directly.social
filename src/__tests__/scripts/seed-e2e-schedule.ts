...
async function main() {
  ...
  if (targetEmail) {
    await seedForUser(targetEmail);
    return;
  }

  console.log('[Global Seed] Seeding schedule for legacy fallback user only...');
  await seedForUser('tester@directly.social');
}
...
async function seedForUser(email: string) {
  ...
  if (!user) {
    console.warn(`[Lazy Seed] User ${email} not found. Skipping schedule seeding.`);
    return;
  }

  console.log(`[Lazy Seed] Seeding schedule data for user: ${email} (${user.id})`);

  const workerSuffix = email === 'tester@directly.social' ? 'legacy' : email.split('@')[0].split('-')[1] || 'legacy';
  ...
  console.log(`[Lazy Seed] Successfully seeded schedule data for ${email}.`);
}
