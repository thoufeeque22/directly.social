...
export function LoginContent({ referrerName }: { referrerName?: string | null }) {
  ...
  const handleGoogleLogin = async () => {
    ...
    const callbackUrl = searchParams.get('callbackUrl') || '/activity';
    ...
    const appHostname = isLocal 
      ? `app.localhost:${window.location.port || 3000}` 
      : isStaging ? `app.staging.directly.social` : `app.directly.social`;
    const protocol = isLocal ? 'http' : 'https';
    
    const redirectUrl = `${protocol}://${appHostname}/auth/v1/callback?next=${callbackUrl}`;
