    ...
    <div data-testid={`history-post-${post.id}`} className={`${styles.postCard} ${isCardActive ? styles.activePost : ''}`}>
      ...
      <GlassCard className={styles.cardInner} style={{ position: 'relative', overflow: 'hidden', opacity: isOptimistic ? 0.7 : 1 }}>
        ...
        <div className={styles.cardHeader} style={((allPending || isOptimistic) && isCardActive) || isActiveStaging ? { paddingTop: '1.75rem' } : {}}>
          ...
          {!isOptimistic ? (
            <HistoryCardActions post={post} isCardActive={isCardActive} isActiveStaging={isActiveStaging} activeResumingId={activeResumingId} onCancelAll={onCancelAll} onInPlaceResume={onInPlaceResume} now={now} />
          ) : isCardActive && (
        ...
        <HistoryPlatformList post={post} cancelledIds={cancelledIds} processingIds={processingIds} onRetry={onRetry} onCancel={onCancelPlatform} now={now} />
      </GlassCard>
    </div>
  );
}
