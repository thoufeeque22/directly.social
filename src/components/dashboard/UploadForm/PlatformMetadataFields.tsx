          ...
          <div key={platform} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            ...
            {limits.title && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                <div style={{ display: 'flex', justifyContent: 'flex-end' }}><span style={{ fontSize: '0.65rem', fontWeight: 600, color: isTitleOver ? 'hsl(var(--destructive))' : 'hsl(var(--muted-foreground))' }}>{titleVal.length}/{limits.title}</span></div>
                <input type="text" placeholder={`${platform} title (optional)...`} value={titleVal} onChange={(e) => handlePlatformTitleChange(platform, e.target.value)} style={{ ...inputStyle, borderColor: isTitleOver ? 'hsl(var(--destructive))' : 'hsla(var(--border) / 0.5)' }} />
              </div>
            )}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}><span style={{ fontSize: '0.65rem', fontWeight: 600, color: isDescOver ? 'hsl(var(--destructive))' : 'hsl(var(--muted-foreground))' }}>{descVal.length}/{limits.description}</span></div>
              <textarea placeholder={`${platform} description (optional)...`} value={descVal} onChange={(e) => handlePlatformDescriptionChange(platform, e.target.value)} rows={2} style={{ ...inputStyle, borderColor: isDescOver ? 'hsl(var(--destructive))' : 'hsla(var(--border) / 0.5)' }} />
            </div>
          </div>
        );
      })}
