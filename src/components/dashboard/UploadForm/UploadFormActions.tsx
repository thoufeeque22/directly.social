...
export const UploadFormActions: React.FC = () => {
  const { isUploading, aiTier, hasCachedPreviews, onResumeReview, onTierChange } = useUploadFormContext();
      ...
      <div style={{ display: 'flex', gap: '1rem' }}>
        {hasCachedPreviews && !isUploading && (
          <button type="button" onClick={onResumeReview} style={secondaryButtonStyle}>
            <SkipNextIcon sx={{ fontSize: 18 }} /> Continue Reviewing
          </button>
        )}
        ...
        <button type="submit" disabled={isUploading} style={{ 
          ...primaryButtonStyle, flex: (hasCachedPreviews || (aiTier === 'Manual' && !isUploading)) ? 1.2 : 1,
