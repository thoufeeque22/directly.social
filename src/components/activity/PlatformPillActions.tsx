...
export function PlatformPillActions({
  p, isFailed, isCancelled, isPending, isOptimistic, isPostStale, activityId, onRetry, onCancel
  ...
  return (
    <div className={styles.pillActions}>
      ...
      {(isPending || isOptimistic) && !isCancelled && !isPostStale && (
        <button className={styles.pillActionButton} onClick={(e) => onCancel(e, p.id, p.platform, activityId)} title="Stop Platform Upload" aria-label="Stop Platform Upload">
