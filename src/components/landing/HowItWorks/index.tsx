...
const StepDescription = ({ text }: { text: string }) => {
  ...
  const [prefix, linkPart] = text.split('[');
  const [label] = linkPart.split(']');
  return (
    <>
      {prefix}
      <Link href="/philosophy" style={{ color: theme.palette.primary.main, fontWeight: 600, textDecoration: 'none' }}>
        {label}
      </Link>
    </>
  );
        ...
        <Grid container spacing={4}>
          {howItWorksSteps.map((step, index) => (
            <Grid size={{ xs: 12, md: 4 }} key={index}>
              <Paper elevation={0} sx={{ p: 4, height: '100%', borderRadius: 3, bgcolor: 'background.default', border: `1px solid ${theme.palette.divider}`, position: 'relative' }}>
                <Typography variant="h1" sx={{ position: 'absolute', top: -20, right: 20, fontSize: '6rem', fontWeight: 900, color: 'primary.main', opacity: 0.05, userSelect: 'none' }}>{step.step}</Typography>
                <Stack spacing={2}>
                  <Box sx={{ width: 40, height: 40, borderRadius: '50%', bgcolor: 'primary.main', color: 'primary.foreground', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '1.2rem' }}>{step.step}</Box>
                  <Typography variant="h5" sx={{ fontWeight: 700 }}>{step.title}</Typography>
                  <Typography variant="body1" color="text.secondary" sx={{ lineHeight: 1.6 }}><StepDescription text={step.description} /></Typography>
                </Stack>
              </Paper>
            </Grid>
          ))}
