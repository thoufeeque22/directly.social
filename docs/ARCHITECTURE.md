...
- **Viewport Configuration:** The main layout in `src/app/layout.tsx` includes `<meta name="viewport" content="viewport-fit=cover">`. This allows the content to expand into the edges of the screen, including the areas behind the status bar and home indicator.
- **Global CSS Variables:** Standard safe area insets are mapped to CSS variables in `src/app/globals.css` for ease of use and consistent overrides:
  ```css
  :root {
  ...
  ```
- **Component Implementation:** Components use `calc()` to combine base padding with these safe area insets. For example, a fixed header with an 80px base height would use:
  ```css
