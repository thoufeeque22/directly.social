...
## Prerequisites
1.  **Database Seeded:** Run `npm run seed:e2e` (or equivalent script that executes `src/__tests__/scripts/seed-e2e-user.ts`) to ensure `tester@directly.social` is a `USER` and `admin@directly.social` is an `ADMIN`.
...
1.  **Login:** Log in as `admin@directly.social`.
...
1.  **Login:** Log in as `tester@directly.social`.
