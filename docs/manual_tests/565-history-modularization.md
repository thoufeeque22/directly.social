# Manual Test: History Modularization (#565)

## Overview
This test verifies that the modularized History domain (Page, Hooks, and Actions) functions correctly across all core user flows, including filtering, pagination, and platform-specific actions.

## Prerequisites
- A running development environment.
- At least one user account with existing post history (past and scheduled).
...
3.  **Verify:** `HistoryHeader` displays the correct title and any global actions.
4.  **Verify:** `HistoryList` renders history cards for existing posts.
5.  **Verify:** Each `HistoryCard` shows the post title, description, and scheduled/created date.
...
5.  Clear all filters.
6.  **Verify:** The full list of history items is restored.

### 3. Pagination
1.  If history exceeds 10 items, navigate to Page 2.
...
### 4. History Card Details & Platforms
...
1.  **Retry:** Locate a failed post. Click the "Retry" button.
    - **Verify:** The retry action is triggered via `useHistoryActions`.
...
3.  **Delete:** Click the delete icon on a history card.
