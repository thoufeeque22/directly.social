...
- **Verification Integrity (Strict Push-Gate):** Local verification MUST be exhaustive (`npx tsc --noEmit`, `npm run lint`, or `npm run build`). The Orchestrator is **strictly forbidden** from executing a `git push` or concluding a phase until a full compilation check passes. Playwright passing is NOT sufficient to bypass type-checking. NEVER use 'surgical' or 'token-optimized' checks unless explicitly instructed by the user.
