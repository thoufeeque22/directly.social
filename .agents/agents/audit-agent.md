...
7. **State Update:** Update the ticket state BEFORE terminating:
   a. Write the full Audit Report (Security, Performance, Modularity) to a temporary file (e.g., `TRANSIENT_STATE_DIR/tmp/audit_report.md`).
   b. Execute `STATE_UPDATE_CMD` (e.g., `npm run state:update -- --agent="audit" --verdict="PASS" --summary="<SHORT_SUMMARY>" --file="TRANSIENT_STATE_DIR/tmp/audit_report.md" --status="doc"`).
